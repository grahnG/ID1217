...
RESULT_FILE="quicksort_pthreads_benchmark.csv"
...
for size in "${ARRAY_SIZES[@]}"; do
    ...
    serial_times=()
    for ((i = 0; i < RUNS; i++)); do
        output=$(./quicksort1 $size 1 | grep "Serial Time" | awk '{print $3}')
        if [[ -z "$output" ]]; then
    ...
    for threads in "${THREAD_COUNTS[@]}"; do
        ...
        parallel_times=()
        for ((i = 0; i < RUNS; i++)); do
            output=$(./quicksort1 $size $threads | grep "Parallel Time" | awk '{print $3}')
            if [[ -z "$output" ]]; then
